...
field = pygame.Rect(window_size[0]/32, window_size[1]/64, window_size[0]/4, (window_size[1]/2 - window_size[1]/32))
...
MENU_BG = pygame.image.load(curDir + img_path + 'backgrounds/MAIN_MENU_BG_V1.png')

# Setup Entities and Objects ------------------------------------------------- #

start_localizations = {'player': [int(field.x + field.width/2), int(field.y + field.height*3/4)], 'pyramyth': [int(field.x + field.width/2), int(field.y + field.height*3/16)]}

player = players.player(start_localizations['player'][0], start_localizations['player'][1], 11, 11, 'player', 3)
pyramyth = entities.entity(start_localizations['pyramyth'][0], start_localizations['pyramyth'][1], 31, 32, 'pyramyth')
...
def menu():
    ...
    click = False
    MENU_BG = pygame.image.load(curDir + img_path + 'backgrounds/MAIN_MENU_BG_V1.png')
    MENU_BG = pygame.transform.scale(MENU_BG, (1152/2, 824/2))
        ...
        for event in pygame.event.get():
            ...
            if event.type == KEYDOWN:
                if event.key == K_ESCAPE:
                    pygame.quit()
                    sys.exit()
                if event.key == K_RETURN: # returns to main menu
                    game()
