...
'''
    NOTE TO SELF: Keep this here for now for reference
'''
...
class physics_obj(object):
    ...
    def move_within(self, movement, bounding_boxes):
        ...
        for box in bounding_boxes:
            if self.rect.colliderect(box):
                ...
                self.y += movement[1]
                self.rect.y = int(self.y)
                if self.rect.bottom >= box.bottom:
                    self.rect.bottom = box.bottom
                    collision_types['bottom'] = True
                elif self.rect.top <= box.top:
...
class entity(object):
    ...
    def __init__(self,x,y,size_x,size_y,e_type,action='idle'): # x, y, size_x, size_y, type
        self.x = x
        self.y = y
        self.size_x = size_x
        self.size_y = size_y
        ...
        self.action_timer = 0
        self.action = ''
        self.set_action(action) # overall action for the entity
        self.entity_data = {}
        self.alpha = None
    ...
    def move_within(self,momentum,platforms):
        collisions = self.obj.move_within(momentum,platforms)
        self.x = self.obj.x
        self.y = self.obj.y
        return collisions
    ...
    def display(self,surface,movement):
        image_to_render = None
        if self.animation == None:
            if self.image != None:
                image_to_render = flip(self.image,self.flip).copy()
        ...
        if image_to_render != None:
            ...
            if self.alpha != None:
                image_to_render.set_alpha(self.alpha)
            surface.blit(image_to_render, (self.x, self.y))
...
def load_animations(path):
    ...
    f = open(path + 'old_entity_animations.txt','r')
    print(str(path + 'old_entity_animations.txt'))
    data = f.read()
    f.close()
